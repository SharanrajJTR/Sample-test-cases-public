...
- Name: add-products-cart.spec.js >> Cart Tests >> add products in cart
- Location: tests\add-products-cart.spec.js:5:3
...
```
Error: page.hover: Test timeout of 60000ms exceeded.
Call log:
  ...
  - attempting hover action
    ...
    - retrying hover action
      - waiting 100ms
    70 × waiting for element to be visible and stable
       - element is visible and stable
       - scrolling into view if needed
       - done scrolling
       - <iframe width="" height="" vspace="0" hspace="0" tabindex="0" id="aswift_3" scrolling="no" name="aswift_3" frameborder="0" marginwidth="0" marginheight="0" title="Advertisement" allowtransparency="true" data-load-complete="true" aria-label="Advertisement" data-google-container-id="a!4" allow="attribution-reporting; run-ad-auction" data-google-query-id="CO7fxtLMkJUDFbjFhAAd1jEY2g" sandbox="allow-forms allow-popups allow-popups-to-escape-sandbox allow-same-origin allow-scripts allow-top-navigation-by-us…></iframe> from <ins tabindex="0" aria-hidden="false" data-ad-status="filled" data-vignette-loaded="true" data-adsbygoogle-status="done" class="adsbygoogle adsbygoogle-noablate">…</ins> subtree intercepts pointer events
                ...
                - iframe [ref=f11e18]:
                  - generic [active] [ref=f17e1]:
                    ...
                    - generic [ref=f17e2] [cursor=pointer]:
                      - link:
                        - /url: javascript:void(0);
                - link "AdChoices arrow" [ref=f11e20] [cursor=pointer]:
                  - /url: https://www.flashtalking.com/consumer-privacy
                  - img "AdChoices arrow" [ref=f11e21]
                - link "Privacy Notification" [ref=f11e23] [cursor=pointer]:
                  - /url: https://www.flashtalking.com/consumer-privacy
                  - img [ref=f11e24]
                - link "Privacy Notification" [ref=f11e26] [cursor=pointer]:
                  - /url: https://www.flashtalking.com/consumer-privacy
                  - img [ref=f11e27]
          - iframe
```
...
```ts
  ...
  3  | 
  4  | test.describe('Cart Tests', () => {
  5  |   test('add products in cart', async ({ page }) => {
  6  |     await allure.epic('Cart Operations');
  7  |     await allure.severity('critical');
  8  | 
  9  |     await allure.step('Launch browser and navigate to url', async () => {
  10 |       await page.goto('/');
  11 |     });
  12 | 
  13 |     await allure.step('Verify that home page is visible successfully', async () => {
  14 |       await expect(page).toHaveTitle(/Automation Exercise/);
  15 |     });
  16 | 
  17 |     await allure.step('Click "Products" button', async () => {
  18 |       await page.click('text=Products');
  19 |     });
  20 | 
  21 |     await allure.step('Hover over first product and click "Add to cart"', async () => {
> 22 |       await page.hover('.product-image-wrapper >> nth=0');
     |                  ^ Error: page.hover: Test timeout of 60000ms exceeded.
  23 |       await page.click('.add-to-cart >> nth=0');
  24 |     });
  25 | 
  26 |     await allure.step('Click "Continue Shopping" button', async () => {
  27 |       await page.click('button:has-text("Continue Shopping")');
  28 |     });
  29 | 
  30 |     await allure.step('Hover over second product and click "Add to cart"', async () => {
  31 |       await page.hover('.product-image-wrapper >> nth=1');
  32 |       await page.click('.add-to-cart >> nth=2'); // Index 2 is often the second product's overlay button
  33 |     });
  34 | 
  35 |     await allure.step('Click "View Cart" button', async () => {
  36 |       await page.click('u:has-text("View Cart")');
  37 |     });
  38 | 
  39 |     await allure.step('Verify both products are added to Cart', async () => {
  40 |       const rows = page.locator('#cart_info_table tbody tr');
  41 |       await expect(rows).toHaveCount(2);
  42 |     });
  43 | 
  44 |     await allure.step('Verify their prices, quantity and total price', async () => {
  45 |       const firstProductPrice = await page.locator('#product-1 .cart_price').innerText();
  46 |       const firstProductQuantity = await page.locator('#product-1 .cart_quantity').innerText();
  47 |       const firstProductTotal = await page.locator('#product-1 .cart_total').innerText();
  48 |       
  49 |       expect(firstProductQuantity).toBe('1');
  50 |       // Additional price verification could be added here
  51 |     });
  52 |   });
  53 | });
  54 | 
